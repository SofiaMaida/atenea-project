package ar.com.ada.atenea.advice;

import ar.com.ada.atenea.exception.ApiErrorsResponseBody;
import ar.com.ada.atenea.exception.ApiFieldError;
...
public class ApiValidationExceptionHandler extends ResponseEntityExceptionHandler {
    ...
    protected ResponseEntity<Object> handleMethodArgumentNotValid(
        ...
        // describen la informacion de los errores
    ...
    protected ResponseEntity<Object> handleHttpMessageNotReadable(HttpMessageNotReadableException ex, HttpHeaders headers, HttpStatus status, WebRequest request) {
        // Se divide el mensaje por los saltos de lineas (\n) y se almacena en una lista de string
        List<String> exMessagelines = Arrays.asList(ex.getMessage().split("\n"));

        // Del primer elemento de la lista exMessagelines (que es una string), se divide por 2 puntos (:)
        // y lo almacena en otra lista de string
        List<String> messageLines = Arrays.asList(exMessagelines.get(0).split(":"));

        // de la lista messageLines, se obtiene el ultimo elemento que nos indica el error que ocurrio
        String message = messageLines.get(messageLines.size() - 1).trim();

        // Del segundo elemento de la lista exMessagelines (que es una string), se divide por 2 doble comillas (\")
        // y lo almacena en otra lista de string
        List<String> filedLines = Arrays.asList(exMessagelines.get(1).split("\""));

        // de la filedLines, se obtiene el segundo elemento que nos indica el campo que tiene la validacion.
